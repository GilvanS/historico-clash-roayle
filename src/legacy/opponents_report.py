...
class OpponentsReporter:
    def __init__(self, api_token: str, db_path: str = None):
        ...
        # Inicializa o gerenciador de CSV e carrega os dados para a memoria/banco
        self.csv_manager = CSVDatabaseManager(db_path=db_path)
        self.csv_manager.load_all_csvs()
        
        # Define o path (pode ser arquivo ou URI de memoria)
        self.db_path = self.csv_manager.db_path
        self.is_uri = not db_path # Se nao tem db_path, é a URI de memoria
        
        # Sincroniza o esquema se necessario (o manager ja cria as tabelas base)
        self.init_database()

    def _get_connection(self):
        """Retorna uma conexao com o banco de dados (lidando com URI/Arquivo)"""
        if self.is_uri:
            return sqlite3.connect(self.db_path, uri=True)
        return sqlite3.connect(self.db_path)
    
        # Se for URI (memoria), precisa do flag uri=True. Se for arquivo, não.
        if self.is_uri:
