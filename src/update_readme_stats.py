#!/usr/bin/env python3
"""
Script para atualizar o README com estatisticas de decks e performance
"""
...
class ReadmeStatsUpdater:
    ...
    def get_deck_stats(self, player_tag: str) -> Dict:
        ...
        for row in cursor.fetchall():
            deck, total, vitorias, derrotas, trofes_ganhos, trofes_perdidos = row
            win_rate = (vitorias / total * 100) if total > 0 else 0
            trofes_total = (trofes_ganhos or 0) + (trofes_perdidos or 0)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Atualiza README com estatisticas de decks')
    parser.add_argument('--player-tag', type=str, help='Tag do jogador (ex: #YVJR0JLY)')
    parser.add_argument('--db', type=str, default='oponentes.db', help='Caminho do banco de dados')
    ...
    updater = ReadmeStatsUpdater(db_path=args.db, readme_path=args.readme)
